...
class ZooDataset(Dataset):
    ...
    def __init__(self, root='zoodata', dataset="joint", split='train', topk=None, num_sample=5, scale=1.0, transform=None, normalize=False,
        ...
        # datapath = os.path.join(root, f'phi_3_weights/Phi-3-mini-4k-instruct_norm_.pt')
        datapath = os.path.join(root, f'llama_weights/llama_3_8_1_norm_dict_instruct_.pt')
        # datapath = os.path.join(root, f'llama3_weights/VAGOsolutions_layer_norm_.pt')

        # weights = torch.load('../../Datasets/phi_3_weights/Phi-3-mini-4k-instruct_norm_.pt')
    ...
    def load_data(self, file, dataset='joint'):
        ...
        xc = torch.load('../../Datasets/llama3_weights/tfblock_llama3-1-8b_normlabels_.pt')
        # xc = torch.load('../../Datasets/llama3_weights/llama3-1-8b_instruct_normlabels_.pt')
        wl = []
        targets = []
        if dataset=='joint':
            keys = list(data)
