...
def initialize_search_records(search_pass_name, particle_paths, eval_type, dataset, gpus, base_model, fast_merge,
    ...
    for i in range(len(particle_paths)):
        shutil.copytree(particle_paths[i], os.path.join("search", search_pass_name, "particle_" + str(i), "now"),
                        dirs_exist_ok=True)
        shutil.copytree(os.path.join("search", search_pass_name, "particle_" + str(i), "now"),
                        os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"),
                        dirs_exist_ok=True)
    ...
    # the default setting
        ...
        for i in range(len(particle_paths)):
            ...
            merge_args.append(([-1, 1], [os.path.join("search", search_pass_name, "particle_" + str(i), "now"),
                                         os.path.join("search", search_pass_name, "particle_" + str(secret_lover_id),
                                                      "now")],
                               os.path.join("search", search_pass_name, "particle_" + str(i), "velocity"),
                               gpus[assign_gpu(len(gpus), i, len(particle_paths))], fast_merge))
    ...
    eval_args = []
    for i in range(len(particle_paths)):
        eval_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "now"), eval_type, dataset,
                          gpus[assign_gpu(len(gpus), i, len(particle_paths))], base_model, True))
    ...
    shutil.copytree(os.path.join("search", search_pass_name, "particle_" + str(best_idx), "now"),
                    os.path.join("search", search_pass_name, "global_best"), dirs_exist_ok=True)

    # initialize global worst checkpoint
    worst_idx = results.index(min(results))
    shutil.copytree(os.path.join("search", search_pass_name, "particle_" + str(worst_idx), "now"),
                    os.path.join("search", search_pass_name, "global_worst"), dirs_exist_ok=True)
...
def particle_update(i, gpu_id, search_pass_name, weight_randomess, inertia, cognitive_coeff, social_coeff, repel_coeff,
    ...
    lora_merge(
        weights=[1, -1],
        lora_name_list=[os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"),
                        os.path.join("search", search_pass_name, "particle_" + str(i), "now")],
    ...
    lora_merge(
        weights=[1, -1],
        lora_name_list=[os.path.join("search", search_pass_name, "global_best"),
                        os.path.join("search", search_pass_name, "particle_" + str(i), "now")],
    ...
    lora_merge(
        weights=[-1, 1],
        lora_name_list=[os.path.join("search", search_pass_name, "global_worst"),
                        os.path.join("search", search_pass_name, "particle_" + str(i), "now")],
    ...
    lora_merge(
        ...
        lora_name_list=[os.path.join("search", search_pass_name, "particle_" + str(i), "now"),
                        os.path.join("search", search_pass_name, "particle_" + str(i), "velocity")],
...
if __name__ == "__main__":
    ...
    argParser.add_argument("-e", "--eval_type",
                           help="evaluation types")  # multiple_choice, exact_match, multitask, rm_default, rm_verbose, rm_concise, human
    argParser.add_argument("-d", "--dataset",
                           help="dataset as the search objective/evaluation")  # file names in data/eval, be mindful of using the right --eval_type
    argParser.add_argument("-g", "--gpus", help="available gpu ids in a string")  # such as 0,1,2,3,4
    argParser.add_argument("--num_cpu_when_merging", default=1,
                           help="number of cpu cores when merging")  # you don't need to change this honestly
    ...
    argParser.add_argument("-i", "--initial_expert_directory", default="./initial_experts",
                           help="initial expert directory")  # make it a directory of initial expert checkpoints, see initial_experts/ for example
    ...
    argParser.add_argument("--fast_merge", default=1,
                           help="whether to use fast merge by only loading the safetensor file")  # just keep it 1 unless you absolutely know what you're doing
    ...
    argParser.add_argument("--initial_experts_num", default=None,
                           help="number of initial experts to populate, when populate flag is 1")
    argParser.add_argument("--starting_velocity_mode", default="random",
                           help="starting velocity mode: zero, random, best")  # zero, random, best
    argParser.add_argument("--repel_term", default=1,
                           help="whether to incorporate a repel term with global_worst")  # 0, 1
    argParser.add_argument("--step_length_factor", default=0.95,
                           help="step length *= step_length_factor every iteration")  # 1 for no scheduling, 0.95 maybe?
    ...
    argParser.add_argument("--restart_patience", default=0.5,
                           help="restart patience * patience = when to restart particles")
    argParser.add_argument("--clean_up_on_end", default=1, help="whether to clean up on end")  # 0, 1
    argParser.add_argument("--only_one_or_two", default=None,
                           help="whether to only optimize with dataset 1 or 2 in multitask")  # safely ignore this
    argParser.add_argument("--to_visualize", default=False,
                           help="whether to visualize the search process")  # 0, 1, for Fig 8
    argParser.add_argument("--correctness_emergence", default=False,
                           help="whether to track correctness changes wrt iteration")  # 0, 1, for Fig 2
    ...
    initialize_search_records(search_pass_name, particle_paths, eval_type, dataset, gpus, base_model, fast_merge,
                              starting_velocity_mode)
    ...
    if starting_test_set_eval:
        ...
        for i in range(len(particle_paths)):
            eval_test_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "now"), eval_type,
                                   dataset, gpus[assign_gpu(len(gpus), i, len(particle_paths))], base_model))
    ...
    while iter_count < max_iteration:
        ...
        if to_visualize_flag:
            for i in range(len(particle_paths)):
                lora_weight_path = os.path.join("search", search_pass_name, "particle_" + str(i), "now",
        ...
        for i in range(len(particle_paths)):
            ...
            update_args.append((i, gpus[assign_gpu(len(gpus), i, len(particle_paths))], search_pass_name,
                                weight_randomess, inertia, cognitive_coeff, social_coeff, repel_coeff, fast_merge,
                                step_length, repel_term, restart_flag))
        ...
        for i in range(len(particle_paths)):
            ...
            if not correctness_emergence:
                eval_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "now"), eval_type,
                                  ...
                                  global_skip_flag or local_skip_flag))
            else:
                eval_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "now"), eval_type,
    ...
    for i in range(len(particle_paths)):
        eval_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"), eval_type,
                          dataset, gpus[assign_gpu(len(gpus), i, len(particle_paths))], base_model, True))
    ...
    eval_test_args = []
    for i in range(len(particle_paths)):
        eval_test_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"),
                               eval_type, dataset, gpus[assign_gpu(len(gpus), i, len(particle_paths))], base_model))
    ...
    if eval_type == "perplexity" or eval_type == "multitask":
        ...
        for i in range(len(particle_paths)):
            eval_test_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"),
                                   ...
                                   gpus[assign_gpu(len(gpus), i, len(particle_paths))], base_model))
        ...
        final_metrics["ending_best_single_test_" + dataset_1_name] = results[
            final_metrics["ending_best_particle_on_validation"]]
        ...
        final_metrics["ending_best_single_test_" + dataset_2_name] = results[
            final_metrics["ending_best_particle_on_validation"]]
    ...
    if eval_type == "multitask":
        ...
        eval_args = []
        for i in range(len(particle_paths)):
            eval_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"),
                              "multiple_choice", dataset_1_name, gpus[assign_gpu(len(gpus), i, len(particle_paths))],
                              base_model))
        ...
        final_metrics["ending_best_single_dev_" + dataset_1_name] = results[
            final_metrics["ending_best_particle_on_validation"]]
        ...
        for i in range(len(particle_paths)):
            eval_args.append((os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best"),
                              "multiple_choice", dataset_2_name, gpus[assign_gpu(len(gpus), i, len(particle_paths))],
                              base_model))
        ...
        final_metrics["ending_best_single_dev_" + dataset_2_name] = results[
            final_metrics["ending_best_particle_on_validation"]]
    ...
    try:
        for i in range(len(particle_paths)):
            ...
            os.rename(os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best", "golds_dev.json"),
                      os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best", "golds.json"))
            os.rename(os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best", "preds_dev.json"),
                      os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best", "preds.json"))
            os.rename(os.path.join("search", search_pass_name, "particle_" + str(i), "now", "golds_dev.json"),
                      os.path.join("search", search_pass_name, "particle_" + str(i), "now", "golds.json"))
            os.rename(os.path.join("search", search_pass_name, "particle_" + str(i), "now", "preds_dev.json"),
                      os.path.join("search", search_pass_name, "particle_" + str(i), "now", "preds.json"))
    except:
        for i in range(len(particle_paths)):
            ...
            os.rename(
                os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best", "scores_dev.json"),
                os.path.join("search", search_pass_name, "particle_" + str(i), "personal_best", "scores.json"))
            os.rename(os.path.join("search", search_pass_name, "particle_" + str(i), "now", "scores_dev.json"),
                      os.path.join("search", search_pass_name, "particle_" + str(i), "now", "scores.json"))
