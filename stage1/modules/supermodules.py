...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 2), num_res_blocks,
        ...
        self.fc_in = LinearSuper(self.in_dim, fdim)
    ...
    def forward(self, x):
        ...
        #adapt input shape
        b = x.shape[0]
        x = x.reshape(b, self.fch, -1)
        in_dim = x.shape[-1]
        self.fc_in.samples(in_dim, self.fdim)
        x = self.fc_in(x)
        x = F.leaky_relu(x)
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        # h = h.reshape((-1, self.in_channels, 256, 256))
        # h = torch.sigmoid(h)
        # h = torch.tanh(h)
        if self.tanh_out:
            h = torch.tanh(h)
