        ...
        *,
        ...
        for i in range(layers):
            self.layers.append(nn.ModuleList([
                ...
                # nn.Conv1d(dim, dim/2**(i+1), 3, padding=1, bias = False),
            ]))
        ...
        *,
        ...
        for i in range(layers):
            self.layers.append(nn.ModuleList([
                Attention(dim = dim, dim_head = dim_head, heads = heads),
                FeedForward(dim = dim, mult = ff_mult),
                nn.ConvTranspose1d(in_ch*2**(i+1), in_ch*2**(i+2), 2, bias = False),
            ]))
            self.downsamples.append(nn.ModuleList([
                nn.Conv1d(in_ch / 2 ** i, in_ch/ 2 ** (i + 1), 3, stride=2, padding=1, bias=False),
            ]))
            # self.norm.append(nn.ModuleList([nn.LayerNorm(dim * 2 ** (i + 1)), ]))
            dim = dim / 2 ** (i + 1)
        self.norm = nn.LayerNorm(dim/2**layers)

    def forward(self, x):
        for attn, ff, down in zip(self.layers, self.downsamples):
            x = attn(x) + x
            x = ff(x) + x
            x = down(x)
        return self.norm(x)
...
class UpTransformer(nn.Module):
        ...
        *,
        ...
        for i in range(layers):
            ...
            self.upsamples.append(nn.ModuleList([
                nn.ConvTranspose1d(in_ch* 2 ** i, in_ch *2 ** (i + 1), 4, stride=2, padding=1, bias=False),
            ]))
    ...
    def forward(self, x):
        for attn, ff, up in zip(self.layers, self.upsamples):
            x = attn(x) + x
            x = ff(x) + x
            x= up(x)
