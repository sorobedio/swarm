...
# Define Student's T Latent Regularization Loss
def student_t_kl_loss(z, df):
    return torch.mean((df + 1) / 2 * torch.log(1 + (z ** 2) / df))
...
class TAutoencoder(nn.Module):
    ...
    def forward(self, input):
        ...
        z, df = self.encode(input)
        dec = self.decode(z)
        return input, dec, z, df
