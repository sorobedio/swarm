...
##############################ffn###################################
    ...
    autoencoder = torch.load('./autocheckpoints/llama_model_img_chunk_.pth', map_location='cpu')
    # autoencoder = torch.load('./autocheckpoints/llama_model_chunk_full_block_7first.pth', map_location='cpu')
    torch.save(autoencoder.state_dict(), f'checkpoints/stage1/img_base_chunk_llama_v1.ckpt')
    ...
    for layer in layers:
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=use_amp):
            ...
            for w in tqdm(weight):
                ...
                #
                # x_rec=(x_rec*std)+mu
                wl.append(x_rec.detach().cpu().reshape(-1, 262144))
