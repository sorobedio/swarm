...
class MyDiffusion(nn.Module):
    def __init__(self, unet_config,
        ...
        self.device = device
        # self.input_adapt = nn.Sequential(nn.Linear(2304, 4096)).to(device)
    ...
    def get_input(self, batch):
        ...
        # z = self.input_adapt(z)
        # x = torch.flatten(x, start_dim=1)
        # z = self.input_adapt(x)
        # z = x.reshape(-1, 1, 64, 64)
        z = z.reshape(-1, self.channels, self.input_size, self.input_size)
