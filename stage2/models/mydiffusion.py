...
class MyDiffusion(nn.Module):
    def __init__(self, unet_config,
        ...
        self.input_adapt = nn.Sequential(nn.Linear(in_dim, self.input_size*self.input_size)).to(device)
    ...
    def get_input(self, batch):
        ...
        x = x.to(self.device)
        x = x.reshape(-1, self.channels, self.in_dim)
    ...
    def sample(self, n=1, cond=None, cfg_scale=3):
        ...
        #     cond =[cond]
        with torch.no_grad():
            x = torch.randn((n, self.channels, self.in_dim)).to(self.device)
            x = x.reshape(-1, self.channels, self.in_dim)
            x = self.input_adapt(x)
            x = x.reshape(-1, self.channels, self.input_size, self.input_size)
    ...
    def get_loss(self, pred, target, mean=True):
        if self.loss_type == 'l1':
            loss = (target - pred).abs()/self.logvar.exp()+ self.logvar
            if mean:
                loss = loss.mean()
...
class DiffusionWrapper(nn.Module):
    def __init__(self, diff_model_config, conditioning_key, inchannels=1, inz=96, in_dim=10853):
        super().__init__()
        ...
        # self.input_adapt = nn.Sequential(nn.Linear(in_dim, inz*inz))
        self.output_adapt = nn.Sequential(nn.Linear(inz*inz, in_dim))
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        ...
        # x = self.input_adapt(x)
        # x = x.reshape(-1, self.in_channels, self.inz, self.inz)
        print(x.shape)
        ...
        out = out.reshape(-1, self.in_channels , self.in_size *self.in_size)
        out = self.output_adapt(out)
        out = out.reshape(-1, self.in_channels* self.in_dim)
