...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        # default="stage1/configs/vit_lora_config_kl.yaml",  #
        # default="stage1/configs/c100_resnet18_config_kl.yaml",  #
...
if __name__ == "__main__":
    ...
    # model_list = ["meta-llama/Llama-3.2-1B-Instruct", "meta-llama/Llama-3.2-3B-Instruct"]
    #
    # # model_id= "meta-llama/Meta-Llama-3.1-8B-Instruct"
    # # model_id = "meta-llama/Llama-3.2-1B-Instruct"
    # conds = torch.load('../Datasets/llmdatallama_3_1_3B_norm_fulll_labels.pt')
    ...
    #
    # std = torch.load('./dit_checkpoints/checkpoint_mdit_llama_3_1_2_nrom_epoch=60969_.ckpt')['state_dict']
    ...
    # torch.save(wd, 'wdata/mdt_sampled_weights_50_norm.pt')
    # del ldmmodel
    # del xc
    model_id = "meta-llama/Llama-3.2-1B-Instruct"
    ...
    # wd = torch.load('wdata/llama3_hella_swarm_weights_.pt')
    wd = torch.load('wdata/mdt_sampled_weights_50_norm.pt')
    #f"./wdata/pythia_swarm_global_best_top_1.pt"
    layers =["Llama-3.2-1B-Instruct"]
    utility_values = []
    for i in range(num_samples):
        ...
        lm_eval_model = HFLM(model, device=device, batch_size=64)
        ...
        header = f"\n{'*' * 40}\n{current_date}\n{'*' * 40}\n"
        file_path = 'logfiles/arc_llama3_3.1_results_norm.txt'
        # Append the header and the Markdown table to the file
        with open(file_path, 'a') as file:
            file.write(header)
            file.write(mtable)
            file.write('\n')  # Add a new line at the end
            file.write(f'--iteration--{i}------')  # Add a new line at the end
            file.write('\n')  # Add a new line at the end
    ...
    torch.save(utility_values, 'utility_values_arc_c.pt')
        # logging.info('****************************************')
    ...
    # acc = results['results']['arc_challenge']['acc_norm,none']
    # acc = results['results']['leaderboard_gpqa_main']['acc_norm,none']
    # acc = results['results']['leaderboard_mmlu_pro']['acc,none']
# - leaderboard_bbh (3)
# - leaderboard_gpqa(0)
